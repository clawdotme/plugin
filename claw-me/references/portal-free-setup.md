...
3. For an owner-supplied Claw Me promotion, pass `promo_code` to `billing_start_checkout`. `STAYGRITTY` applies 100% off Plus for the life of that subscription; usage wallet charges and add-ons remain payable. Enter this code in Claw Me or through the Agent, because the shared Stripe account has a separate legacy code with the same name. Stripe displays the final amount before confirmation.
4. Call `billing_start_checkout` with `{"plan":"basic","cloud_claw":false,"idempotency_key":"UNIQUE_APPROVED_PURCHASE_ID"}`. It returns `payment_url` and `payment_id`. Show the Stripe URL for payment confirmation; do not collect raw card data. Merely creating Checkout does not activate the account.
