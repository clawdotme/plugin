...
3. Follow [the final billing step](onboarding.md#final-billing-choice): read the live catalog and offer Free, PAYG when available, Basic, and Plus. Describe recurring fees, usage and add-ons before the owner decides. Free needs no card. PAYG card setup charges nothing; it has separate wallet funding and does not include all subscription allowances.
4. Record the selected choice before hosted setup or Checkout, and reuse the returned `checkout_key` as `idempotency_key`. For a subscription, call `billing_start_checkout` with the approved `plan` and `cloud_claw:false` unless that add-on was separately approved. For PAYG use `billing_start_payg`. Show the returned Stripe URL; never collect card data.
5. Poll `billing_payment_status` for subscription Checkout, or `billing_access_status` for PAYG. Read account activation as well as payment status. Do not create another purchase to resolve a delayed webhook. Confirm the final onboarding choice only after the backend verifies the matching access.
...
An owner-supplied promotion goes in `promo_code` on subscription Checkout, not PAYG setup. `STAYGRITTY` applies 100% off Plus for that subscription's lifetime; wallet usage and add-ons remain payable. Stripe displays the final amount before confirmation.
