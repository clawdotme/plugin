...
The initial landing offers Manual Setup or Ask Your Agent. The final-choice rollout changes the four dashboard steps to Address, Features and permissions, Connect your Agent, and Billing choice, followed by Getting Started. The owner explicitly chooses Free, catalog-enabled pay as you go, Basic, or Plus. Completion requires backend-verified matching access; Free needs no card. Pending choices survive Stripe cancellation or delayed activation. This draft requires the matching service rollout; discover live availability before using the new billing tools. An address and owner-confirmed permissions are required; Agent connection is optional afterward. New Incoming email choices default to Store for me only, and public sharing defaults off. Preserve existing choices. Completed accounts enter their workspace by default. See [the onboarding reference](claw-me/references/onboarding.md) for the server-driven Agent interview and secure authorization boundaries.
